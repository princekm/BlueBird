...
import com.example.sprites.BorderSprite;
import com.example.sprites.PipeSprite;
...
public class GameScene extends Scene implements IOnSceneTouchListener {
			...
			ContactListener contactListener = new ContactListener() {
				...
				public void beginContact(Contact contact) {
					...
					if (!GAME_OVER) {
						stopPipes();
						GAME_OVER = true;
						manager.prefs.checkAndSetHscore(score);
	...
	private void createUpdateScoreHandler() {
		point = new TimerHandler(1.5f, true, new ITimerCallback() {
	...
	private void createDeleteHandler() {
		del = new TimerHandler(0.2f, true, new ITimerCallback() {
			public void onTimePassed(TimerHandler pTimerHandler) {
	...
	public boolean onSceneTouchEvent(Scene pScene, TouchEvent pSceneTouchEvent) {
		if (pSceneTouchEvent.isActionDown() && !GAME_OVER) {
			sPlayer.getBody().setLinearVelocity(new Vector2(0, -5));
	...
	private void setParallax() {
		setBackground(manager.RM.autoParallaxBackground);
	}
